    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length == 1) {
            ...
            if (args[0].equals("생성")) {
                ...
                completions.add("<랜덤박스ID>");
                completions.addAll(RandomBox.getInstance().getRandomBoxRepository().getAllRandomBox().stream().map(net.starly.randombox.randombox.RandomBox::getName).collect(Collectors.toList()));
            }
        } else if (args.length == 3) {
            if (args[0].equals("지급")) {
                completions.add("<플레이어>");
                completions.add("@a");
                completions.addAll(RandomBox.getInstance().getServer().getOnlinePlayers().stream().map(Player::getName).collect(Collectors.toList()));
            }
        }
